...
export function parseAIResponseToSubGoalsNoMarkDown(aiResponse) {
  if (!aiResponse) return [];

  // 1. 预处理：尝试提取 JSON 数组部分
  let cleanResponse = aiResponse.trim();
  
  // 移除可能存在的 Markdown 代码块标记 (如 ```json ... ```)
  ...
  if (jsonMatch) {
    try {
      ...
      if (Array.isArray(jsonData)) {
        return jsonData
          .filter(item => item && (typeof item === 'string' || item.message || item.step))
          .map((item, index) => ({
            _id: `node-${Date.now()}-${index}`,
            message: typeof item === 'string' ? item : (item.message || item.step || JSON.stringify(item)),
            finish: false,
            fileList: []
          }));
      }
    } catch (e) {
      console.warn('JSON 提取解析失败，尝试备选解析方案:', e);
    }
  }

  // 2. 备选方案：如果 JSON 解析失败，则使用原有的文本行解析逻辑
  const subGoals = [];
